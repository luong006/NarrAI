...
export function AICopilotPanel({
  ...
  return (
    <aside className="w-80 h-screen border-l border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex flex-col shrink-0 p-4 select-none overflow-y-auto transition-colors">
      ...
      {selectedText && (
        <div className="mb-4 p-3.5 rounded-xl border border-indigo-200 dark:border-indigo-900/60 bg-indigo-50/50 dark:bg-indigo-950/30">
      ...
      {proposedText && (
        <div className="mb-4 p-3.5 rounded-xl border border-emerald-200 dark:border-emerald-900/60 bg-emerald-50/40 dark:bg-emerald-950/30">
      ...
      {/* Default Tips */}
      {!selectedText && !proposedText && (
        <div className="p-4 rounded-xl border border-slate-200/80 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-800/40 text-xs text-slate-600 dark:text-slate-400 space-y-2 mb-4">
          <p>{t.ai_welcome_1}</p>
      ...
      {/* Status indicator */}
      {(loading || streaming) && (
        <div className="p-3 rounded-lg bg-indigo-50 dark:bg-indigo-950/40 border border-brand-200 dark:border-brand-900 flex items-center gap-2 text-xs text-brand-700 dark:text-brand-300 mb-4">
          <div className="w-2 h-2 rounded-full bg-brand-500 animate-pulse" />
          <span>{streaming ? (lang === 'vi' ? "AI đang chấp bút thời gian thực..." : "AI is writing chapter live...") : t.ai_thinking}</span>
        </div>
      )}
      ...
      <div className="mt-auto pt-4 border-t border-slate-200 dark:border-slate-800 space-y-2">
        <button
          ...
          className="w-full py-3 rounded-xl text-xs font-bold text-white bg-brand-700 hover:bg-brand-800 dark:bg-brand-600 dark:hover:bg-brand-700 shadow flex items-center justify-center gap-2 transition-all disabled:opacity-50"
        >
        ...
        <button
          onClick={onEndStory}
          disabled={loading || streaming}
          className="w-full py-2.5 rounded-xl text-xs font-semibold text-red-700 dark:text-red-400 border border-red-200 dark:border-red-900/50 hover:bg-red-50 dark:hover:bg-red-950/40 flex items-center justify-center gap-1.5 transition-colors disabled:opacity-50"
        >
